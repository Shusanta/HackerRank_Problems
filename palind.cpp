...
string makePal(string &n_Val, int count,int latest) {
	string pal(count, ' ');
	int place = 0;
	int single = 0;
	int half = (count / 2.0);
	...
	if (latest == 0)
	pal[half] = n_Val[n_Val.length()-1];
	else {
		pal[half] = n_Val[latest];
	}
	return pal;
...
string storeValue(string &val1, string &val2) {
	...
	for (int i = 0, j = 1; j<n_Val.length();) {
		...
		i = i + 2;
		j = j + 2;
		if (j>n_Val.length() && single == 0 && i < n_Val.length()) {
			single = 1;
			count++;
			latest = i;
		}
	...
	palindrome = makePal(n_Val, count,latest);
	return palindrome;
}
